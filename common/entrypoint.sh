...
ROOT_PRE_CMD_EXEC=${ROOT_PRE_CMD_EXEC:-""}
USER_PRE_CMD_EXEC=${USER_PRE_CMD_EXEC:-""}

if [ -n "$ROOT_PRE_CMD_EXEC" ]; then
    echo "[entrypoint] Running pre command: $ROOT_PRE_CMD_EXEC"
    bash -c "$ROOT_PRE_CMD_EXEC"
fi
...
if [ "$PUID" -ne 0 ] && [ "$PGID" -ne 0 ]; then
    ...
    if [[ -n $USER_PRE_CMD_EXEC ]]; then
        echo "[entrypoint] User-hook: $USER_PRE_CMD_EXEC"
        if [[ $PUID -eq 0 && $PGID -eq 0 ]]; then
            bash -c "$USER_PRE_CMD_EXEC"
        else
            gosu "$USERNAME" bash -c "$USER_PRE_CMD_EXEC"
        fi
    fi
fi
